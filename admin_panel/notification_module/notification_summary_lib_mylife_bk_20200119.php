...
//$notificationSummary = json_decode(file_get_contents($notificationSummaryUrl . $condition));

$apiHitUrl = $notificationSummaryUrl . $condition;
$notificationSummaryData = json_decode(file_get_contents($apiHitUrl));
/*echo "<pre>";
print_r(file_get_contents($apiHitUrl));
exit;*/
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <form action="#">
                                                ...
                                                <div class="col-lg-3 col-md-3 col-sm-3 col-xs-6">
                                                    <a href="notification_summary.php" <span
                            ...
                            <div class="adv-table">
                                <table class="display table table-bordered dataTable" id="my_dynamic_table">
                                    ...
                                    <tr class="">
                                        ...
                                        <th width="5%">WriteTime</th>
                                        <th width="5%">SentTime</th>

                                    </tr>
                                    </thead>
                                    <tbody>
                                    <?php
                                    // echo "<pre>";print_r($notificationSummary->aaData);exit();
                                    if (!empty($notificationSummaryData->aaData)) { ?>
                                        <?php $sl = 0;
                                        foreach ($notificationSummaryData->aaData as $key => $row) {

                                            $msisdn = !empty($row->MSISDN) ? $row->MSISDN : "";
                                            /* $text = !empty($row->SMSText) ? limit_word($row->SMSText ,'5') : "";
                                             $image = !empty($row->image_path) ? limit_word($row->image_path,'5') : "";
                                             $audio = !empty($row->audio_path) ? limit_word($row->audio_path ,'2'): "";
                                             $video = !empty($row->video_path) ? limit_word($row->video_path ,'2') : "";*/

                                            $text = !empty($row->SMSText) ? wordwrap($row->SMSText, '20', "<br>") : "";
                                            $image = !empty($row->image_path) ? wordwrap($row->image_path, '20', "<br>", true) : "";
                                            $audio = !empty($row->audio_path) ? wordwrap($row->audio_path, '20', "<br>", true) : "";
                                            $video = !empty($row->video_path) ? wordwrap($row->video_path, '20', "<br>", true) : "";
