                            ...
                            <div class="adv-table">
                                <table class="display table table-bordered" id="my_data_table">
                                    ...
                                    <tr class="">
										...
										<th width="5%">Quiz Type</th>
										<th width="5%">Quiz Rules</th>
                                        <th width="5%">Image</th>
                                    ...
                                    <?php if (!empty($getQuizList)) { ?>
                                        ...
                                        foreach ($getQuizList as $key => $row) {
											...
											$quiz_type = !empty($row->payment) ? $row->payment : NULL;
											$quiz_rules = !empty($row->quiz_rules) ? $row->quiz_rules : NULL;
											...
											$ghoori_marchant_id = !empty($row->ghoori_marchant_id) ? $row->ghoori_marchant_id : "";
                                            $updateUrl = baseUrl('services_management/edit_quiz.php?updateId=' . $quiz_id . "&quiz_title=" . urlencode($quiz_title) . "&quiz_description=" . urlencode($quiz_description) . "&quiz_image=" . $quiz_image . "&question_limit=" . $question_limit . "&Bench_mark_point=" . $Bench_mark_point. "&ghoori_marchant_id=" . $ghoori_marchant_id);
                                            $updateUrl .= "&quiz_status=" . $quiz_status . "&quiz_category=" . $_REQUEST['quiz_category_id'] . "&quiz_charge=$charging_amount&start_date=$quiz_start_time&end_date=$quiz_end_time&quiz_type=$quiz_type&quiz_rules=" . urlencode($quiz_rules);
                                            ...
                                            <tr class="">
												...
												<td><?php echo $ghoori_marchant_id; ?></td>
                                                <td>
