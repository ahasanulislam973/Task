...
$pageTitle = "Quiz Charging Log";
$tabActive = "quiz_reporting";
$subTabActive = "quiz_charging_log";
...
$quizChargingLog = json_decode(file_get_contents($quizChargingLogUrl . $condition));
            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <section class="panel">
                        ...
                        <?php include 'quizLogFilterModal.php'; ?>
                        ...
                        <div class="panel-body">
                            <div class="adv-table">
                                <table class="display table table-bordered dataTable" id="my_dynamic_table">
                                    ...
                                    <tbody>
                                    <?php if (!empty($quizChargingLog)) { ?>
                                        <?php
                                        foreach ($quizChargingLog as $key => $row) {
                                            ...
                                            $requestTime = !empty($row->created_at) ? $row->created_at : "";
                                            $amount = !empty($row->amount) ? $row->amount : "";
                                            $chargingStatus = !empty($row->charging_status) ? $row->charging_status : "";
                                            ...
                                            <tr class="gradeX">
                                                ...
                                                <td>
                                                    <?php if (strtoupper($chargingStatus) == 'SUCCESSFUL') { ?>
                                                        <span class="label label-success"> <?php echo $chargingStatus ?></span>
                                                    <?php } else { ?>
                                                        <span class="label label-danger"><?php echo $chargingStatus ?></span>
                                                    <?php } ?>
                                                </td>
                                                <td>
                                                    <?php echo $remarks; ?>
    ...
    <script type="text/javascript">
        ...
        $('#btnDiv').on('click', '#export_data', function () {
           ...
           // var base_url = 'https://' + window.location.host + '/BOT/export_data/';
            var base_url = '<?php echo $quizChargingLogExprtUrl;?>';
