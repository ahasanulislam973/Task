...
$pageTitle = "voting_poll_result";
$tabActive = "newspaper_reporting";
$subTabActive = "voting_poll_result";
...
$cn = connectDB();
$isVoting = isVotingPollQuiz($cn, $organizationId);
// $votingPollId = $votingPollQuizId_ProthomAlo;
$votingPollId = $isVoting['quiz_id'];
...
if (isset($_REQUEST['question_id']) && $_REQUEST['question_id'] <> '') {
    $condition .= "&question_id=" . $_REQUEST['question_id'];
    $_SESSION['successMsg'] = "question_id: $_REQUEST[question_id] ";
}


if (isset($votingPollId) && $votingPollId <> '') {
    $condition .= "&quiz_id=" . $votingPollId;
...
//echo $votingPollResult . $condition;
$quizLog = json_decode(file_get_contents($votingPollResult . $condition));
/*print "<pre>";
print_r($quizLog);
print "</pre>";
exit;*/
                                        ...
                                        <form action="#">
                                            ...
                                            <div class="form-group col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                                ...
                                                <input class="form-control" type="text" id="question_id"
                                                       name="question_id"
                                                       value="<?php if (isset($_REQUEST['question_id'])) echo $_REQUEST['question_id']; ?>">
                                            </div>
                                            ...
                                            <div class="col-lg-3 col-md-3 col-sm-3 col-xs-6">
                                                <a href="voting_poll_result.php" <span
                                ...
                                <table class="display table table-bordered dataTable" id="my_dynamic_table">
                                    ...
                                    <?php if (!empty($quizLog)) { ?>
                                        ...
                                        foreach ($quizLog as $key => $row) {
                                            ...
                                            $percentage = !empty($row->percentage) ? $row->percentage : "";
                                    
                                            $question_id = !empty($row->question_id) ? $row->question_id : "";
											$question_text = !empty($row->question_text) ? $row->question_text : "";
                                            $userAns = !empty($row->user_ans) ? $row->user_ans : "";
                                           
                                            $startTime = !empty($row->DATE) ? $row->DATE : "";
                                            ?>
                                            <tr class="gradeX">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#btnDiv').on('click', '#export_data', function () {
                ...
                var base_url = '<?php echo $pollresultExprtUrl;?>';

                var first_date = $("#first_date").val();
                var last_date = $("#last_date").val(); 
                var question_id = '<?php if (isset($_REQUEST['question_id'])) echo $_REQUEST['question_id']; ?>'; 
				//var quiz_id = '<?php if (isset($_REQUEST['quiz_id'])) echo $_REQUEST['quiz_id']; ?>';
				var quiz_id = '<?php  echo $quiz_id?>';
                window.open(base_url + '?first_date=' + first_date + "&last_date=" + last_date + '&question_id=' + question_id + '&quiz_id=' + quiz_id);
