...
$cn = connectDB();
$qry = "SELECT `profile_id`, `msisdn`, `msisdn_payment`, `person_name`, `address` as order_address, `created_at`, updated_at, status FROM `UserProfile` where msisdn_payment is not null ORDER BY updated_at DESC";
$rs = Sql_exec($cn, $qry);
$resultArr = array();
...
<!--                                        <th width="10%">Mobile No.</th>-->
                                    ...
                                    <?php if (!empty($newspaperAtHomeOffice)) { ?>
                                        ...
                                        foreach ($newspaperAtHomeOffice as $key => $row) {
                                            ...
                                            $updatedBy = !empty($row->updated_by) ? $row->updated_by : "";
                                            $status = !empty($row->status) ? $row->status : "";
                                            $regDate = !empty($row->created_at) ? longDateHuman($row->created_at, 'date_time') : "";
